...
def test_check_manual_permissions(strategy, sett, governance, want, deployer, locker, strategist):
  rando = accounts[6]
  ...
  with brownie.reverts():
    strategy.manualRebalance({"from": rando})
  ...
  with brownie.reverts():
    strategy.manualRebalance({"from": strategist})
