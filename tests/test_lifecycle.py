...
def test_setting_min_impacts_ratio_locked(strategy, sett, governance, want, deployer, locker):
  rando = accounts[6]
  ...
  assert locker.lockedBalanceOf(strategy) >= startingBalance / 2 ## 50% is locked (due to rounding between cvx and bcvx we use >=)
